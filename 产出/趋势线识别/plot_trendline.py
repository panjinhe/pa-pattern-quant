...
def _pattern_title(side: str) -> str:
    return "趋势线向下跌破" if side == "short" else "趋势线向上突破"


def _build_selection_score(trade: dict) -> float:
    height_score = min(float(trade["channel_height_atr"]), 10.0) / 10.0
    touch_score = min(
        (int(trade["support_touch_count"]) + int(trade["resistance_touch_count"])) / 6.0,
        1.0,
    )
    measured_move_bonus = 0.20 if trade.get("measured_move_hit_label") else 0.0
    failure_penalty = 0.10 if trade.get("failure_label") else 0.0
    return height_score * 0.55 + touch_score * 0.35 + measured_move_bonus - failure_penalty
...
def _pick_examples(trades_df: pl.DataFrame, target_count: int = DEFAULT_CASE_COUNT) -> list[dict]:
    ...
    for example_no, record in enumerate(selected, start=1):
        ...
        record["example_file"] = f"trendline-example-{example_no:03d}.png"
    return selected
...
def _add_trade_panel(
    ...
    window_start = int(trade["window_start_idx"])
    signal_idx = int(trade["signal_idx"])
    exit_idx = int(trade["exit_idx"])
    window_start_row = _row_at_idx(plot_df, window_start)
    signal_row = _row_at_idx(plot_df, signal_idx)
    exit_row = _row_at_idx(plot_df, exit_idx)
    if window_start_row and signal_row:
        fig.add_shape(
            type="rect",
            x0=window_start_row["timestamp"],
            x1=signal_row["timestamp"],
            y0=float(plot_df["low"].min()),
            y1=float(plot_df["high"].max()),
        ...
        _add_boundary_line(
            fig=fig,
            row=row,
            start_x=window_start_row["timestamp"],
            end_x=signal_row["timestamp"],
            start_y=float(trade["support_line_start"]),
            end_y=float(trade["support_line_current"]),
            color=support_color,
        )
        _add_boundary_line(
            fig=fig,
            ...
            start_x=window_start_row["timestamp"],
            end_x=signal_row["timestamp"],
            start_y=float(trade["resistance_line_start"]),
            end_y=float(trade["resistance_line_current"]),
            color=resistance_color,
        )

        if exit_row:
            bars_after_signal = exit_idx - signal_idx
            support_exit = float(trade["support_line_current"]) + float(trade["trend_slope"]) * bars_after_signal
            resistance_exit = float(trade["resistance_line_current"]) + float(trade["trend_slope"]) * bars_after_signal
            _add_boundary_line(
                fig=fig,
                row=row,
                start_x=signal_row["timestamp"],
                end_x=exit_row["timestamp"],
                start_y=float(trade["support_line_current"]),
                end_y=support_exit,
                color=support_color,
                dash="dot",
                width=1.4,
            ...
            _add_boundary_line(
                ...
                start_x=signal_row["timestamp"],
                end_x=exit_row["timestamp"],
                start_y=float(trade["resistance_line_current"]),
                end_y=resistance_exit,
                color=resistance_color,
                dash="dot",
                width=1.4,
            )
    ...
    if signal_row:
        trigger_label = "下破触发" if side == "short" else "上破触发"
        fig.add_trace(
            go.Scatter(
    ...
    key_points = resistance_points if side == "long" else support_points
    if key_points and signal_row:
        _add_connected_points(
            fig=fig,
            row=row,
            points=[
    ...
    fig.add_annotation(
        ...
        text=(
            ...
            f"通道: {trade['channel_type']} | 高度: {float(trade['channel_height_atr']):.2f} ATR<br>"
            f"触碰: 支撑 {int(trade['support_touch_count'])} / 阻力 {int(trade['resistance_touch_count'])}<br>"
            f"斜率: {float(trade['trend_slope_atr']):.4f} ATR/Bar<br>"
...
def _build_trade_figure(plot_df: pl.DataFrame, trade: dict, preset: str) -> go.Figure:
    ...
    fig.update_layout(
        title=f"趋势线案例 #{trade['example_code']} ({preset}) | 入场 {_format_ts(trade['entry_time'])}",
        width=1600,
        height=760,
...
def _cleanup_old_case_images(plots_dir: Path) -> None:
    for path in plots_dir.glob("trendline-example-*.png"):
        path.unlink(missing_ok=True)
    (plots_dir / "trendline-equity-curve.png").unlink(missing_ok=True)
    (plots_dir / "trendline-equity-curve.html").unlink(missing_ok=True)
    (plots_dir / "trendline-examples.html").unlink(missing_ok=True)
    (plots_dir / "trendline-examples-manifest.csv").unlink(missing_ok=True)
    ...
    manifest_rows = [
        {
            ...
            "support_touch_count": int(item["support_touch_count"]),
            "resistance_touch_count": int(item["resistance_touch_count"]),
            "channel_height_atr": round(float(item["channel_height_atr"]), 6),
...
def _write_gallery_html(
    ...
    cards = []
    for item in examples:
        side_label = "下破支撑线" if item["side"] == "short" else "上破阻力线"
        cards.append(
            f"""
            <article class="card">
              ...
              <div class="meta">
                ...
                <p>通道：{html.escape(str(item['channel_type']))}，高度：{float(item['channel_height_atr']):.2f} ATR</p>
                <p>支撑触碰：{int(item['support_touch_count'])}，阻力触碰：{int(item['resistance_touch_count'])}</p>
...
def _plot_equity_curve(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(size=5, color="#355c7d"),
    ...
    html_path = plots_dir / "trendline-equity-curve.html"
    png_path = plots_dir / "trendline-equity-curve.png"
...
def _update_summary_files(
    ...
    plot_summary = {
        ...
        "per_example_png_glob": str((output_dir / "plots" / "trendline-example-*.png").resolve()),
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="绘制趋势线突破案例图。")
...
def main() -> None:
    ...
    for trade in examples:
        start_idx = max(0, int(trade["window_start_idx"]) - LOOKBACK_BARS)
    ...
    manifest_path = plots_dir / "trendline-examples-manifest.csv"
    _write_manifest(examples, manifest_path)
    gallery_html_path = plots_dir / "trendline-examples.html"
    _write_gallery_html(
        examples=examples,
    ...
    print(f"example glob: {(plots_dir / 'trendline-example-*.png').resolve()}")
