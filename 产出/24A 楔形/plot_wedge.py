...
def _add_trade_panel(
    ...
    for push_no, idx_key in enumerate(
        ...
        if push_row.is_empty():
            continue
        marker_y = float(push_row["high"].item()) if side == "short" else float(push_row["low"].item())
        fig.add_trace(
            go.Scatter(
...
def main() -> None:
    ...
    for row, trade in enumerate(examples, start=1):
        start_idx = max(0, int(trade["first_push_idx"]) - lookback)
    ...
    html_path = plots_dir / "wedge-examples.html"
    png_path = plots_dir / "wedge-examples.png"
    fig.write_html(html_path)
    fig.write_image(png_path, scale=2)
