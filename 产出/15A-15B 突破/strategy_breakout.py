...
from detect_breakout import (  # noqa: E402
    BreakoutConfig,
    detect_breakout,
    get_config,
...
class BacktestConfig:
    continuation_stop_buffer_atr: float = 0.12
    failure_stop_buffer_atr: float = 0.12
    min_stop_atr: float = 0.70
    continuation_target_r_multiple: float = 1.80
    continuation_target_range_multiplier: float = 0.90
    failure_target_r_multiple: float = 1.30
    continuation_max_holding_bars: int = 36
    failure_max_holding_bars: int = 24
    cooldown_bars: int = 4
...
def _signal_role(signal_family: str) -> tuple[str, str]:
    mapping = {
        "bull_followthrough": ("long", "上破后续做多"),
        "bear_followthrough": ("short", "下破后续做空"),
        "bull_failed_breakout": ("short", "上破失败做空"),
        "bear_failed_breakout": ("long", "下破失败做多"),
    }
...
def _simulate_exit(
    ...
    end_idx = min(len(data["idx"]) - 1, entry_idx + max_holding_bars)
...
def _build_trade_record(
    ...
    side, signal_role = _signal_role(signal_family)
    ...
    if signal_family == "bull_followthrough":
        measured_move_hit = bool(data["bull_followthrough_measured_move_hit"][signal_idx])
        failure_label = bool(data["bull_followthrough_failure"][signal_idx])
    elif signal_family == "bear_followthrough":
        measured_move_hit = bool(data["bear_followthrough_measured_move_hit"][signal_idx])
        failure_label = bool(data["bear_followthrough_failure"][signal_idx])
    elif signal_family == "bull_failed_breakout":
        measured_move_hit = bool(data["bull_failed_breakout_reversal_hit"][signal_idx])
        failure_label = bool(data["bull_failed_breakout_failure"][signal_idx])
    else:
        measured_move_hit = bool(data["bear_failed_breakout_reversal_hit"][signal_idx])
        failure_label = bool(data["bear_failed_breakout_failure"][signal_idx])
    ...
    return {
        ...
        "range_window_start_idx": int(data["range_window_start_idx"][breakout_idx]),
        "range_window_end_idx": int(data["range_window_end_idx"][breakout_idx]),
        "range_high": round(float(data["range_high_pre"][breakout_idx]), 6),
        "range_low": round(float(data["range_low_pre"][breakout_idx]), 6),
        "range_mid": round(float(data["range_mid_pre"][breakout_idx]), 6),
        "range_height": round(float(data["range_height_pre"][breakout_idx]), 6),
        "range_height_atr": round(float(data["range_height_atr"][breakout_idx]), 6),
        "range_context_at_breakout": bool(data["range_context"][breakout_idx]),
        "support_touch_count": int(data["support_touch_count"][breakout_idx]),
        "resistance_touch_count": int(data["resistance_touch_count"][breakout_idx]),
        "support_touch_1_idx": data["support_touch_1_idx"][breakout_idx],
        "support_touch_2_idx": data["support_touch_2_idx"][breakout_idx],
        "support_touch_3_idx": data["support_touch_3_idx"][breakout_idx],
        "resistance_touch_1_idx": data["resistance_touch_1_idx"][breakout_idx],
        "resistance_touch_2_idx": data["resistance_touch_2_idx"][breakout_idx],
        "resistance_touch_3_idx": data["resistance_touch_3_idx"][breakout_idx],
        "breakout_close_pos": round(float(data["close_pos"][breakout_idx]), 6),
        "breakout_body_ratio": round(float(data["body_ratio"][breakout_idx]), 6),
        "signal_close_pos": round(float(data["close_pos"][signal_idx]), 6),
        "signal_body_ratio": round(float(data["body_ratio"][signal_idx]), 6),
        ...
        "fees": round(fees, 6),
        "measured_move_hit_label": measured_move_hit,
...
def summarize_backtest(trades_df: pl.DataFrame) -> dict[str, Any]:
    if trades_df.is_empty():
        return {
            ...
            "target_exit_count": 0,
            "time_exit_count": 0,
            "bull_followthrough_trades": 0,
            "bear_followthrough_trades": 0,
            "bull_failed_breakout_trades": 0,
            "bear_failed_breakout_trades": 0,
        }
    ...
    return {
        ...
        "bull_followthrough_trades": trades_df.filter(pl.col("signal_family") == "bull_followthrough").height,
        "bear_followthrough_trades": trades_df.filter(pl.col("signal_family") == "bear_followthrough").height,
        "bull_failed_breakout_trades": trades_df.filter(pl.col("signal_family") == "bull_failed_breakout").height,
        "bear_failed_breakout_trades": trades_df.filter(pl.col("signal_family") == "bear_failed_breakout").height,
    }
...
def run_backtest(
    ...
    cols = [
        ...
        "close_pos",
        "body_ratio",
        "range_window_start_idx",
        "range_window_end_idx",
        "range_high_pre",
        "range_low_pre",
        "range_mid_pre",
        "range_height_pre",
        ...
        "bull_followthrough_signal",
        "bear_followthrough_signal",
        "bull_failed_breakout_signal",
        "bear_failed_breakout_signal",
        "bull_followthrough_breakout_idx",
        "bear_followthrough_breakout_idx",
        "bull_failed_breakout_idx",
        "bear_failed_breakout_idx",
        "bull_followthrough_measured_move_hit",
        "bear_followthrough_measured_move_hit",
        "bull_failed_breakout_reversal_hit",
        "bear_failed_breakout_reversal_hit",
        "bull_followthrough_failure",
        "bear_followthrough_failure",
        "bull_failed_breakout_failure",
        "bear_failed_breakout_failure",
    ]
    data = {col: df[col].to_list() for col in cols}

    signals: list[tuple[int, str]] = []
    for family, col_name in [
        ("bull_followthrough", "bull_followthrough_signal"),
        ("bear_followthrough", "bear_followthrough_signal"),
        ("bull_failed_breakout", "bull_failed_breakout_signal"),
        ("bear_failed_breakout", "bear_failed_breakout_signal"),
    ]:
        for idx in df.filter(pl.col(col_name))["idx"].to_list():
            signals.append((int(idx), family))
    signals.sort(key=lambda item: (item[0], item[1]))

    trades: list[dict[str, Any]] = []
    next_available_idx = -1

    for signal_idx, signal_family in signals:
        if signal_idx < next_available_idx:
            continue

        side, _ = _signal_role(signal_family)
        if signal_family == "bull_followthrough":
            breakout_idx = int(data["bull_followthrough_breakout_idx"][signal_idx])
        elif signal_family == "bear_followthrough":
            breakout_idx = int(data["bear_followthrough_breakout_idx"][signal_idx])
        elif signal_family == "bull_failed_breakout":
            breakout_idx = int(data["bull_failed_breakout_idx"][signal_idx])
        else:
            breakout_idx = int(data["bear_failed_breakout_idx"][signal_idx])

        entry_price = float(data["close"][signal_idx])
        atr = max(float(data["atr"][signal_idx]), 1e-9)
        range_high = float(data["range_high_pre"][breakout_idx])
        range_low = float(data["range_low_pre"][breakout_idx])
        range_mid = float(data["range_mid_pre"][breakout_idx])
        range_height = float(data["range_height_pre"][breakout_idx])

        if signal_family == "bull_followthrough":
            stop_anchor = min(float(data["low"][breakout_idx]), range_high)
            stop_price = min(
                stop_anchor - backtest_config.continuation_stop_buffer_atr * atr,
                entry_price - backtest_config.min_stop_atr * atr,
            )
            risk_per_unit = entry_price - stop_price
            target_distance = max(
                range_height * backtest_config.continuation_target_range_multiplier,
                risk_per_unit * backtest_config.continuation_target_r_multiple,
            )
            target_price = entry_price + target_distance
            max_holding = backtest_config.continuation_max_holding_bars
        elif signal_family == "bear_followthrough":
            stop_anchor = max(float(data["high"][breakout_idx]), range_low)
            stop_price = max(
                stop_anchor + backtest_config.continuation_stop_buffer_atr * atr,
                entry_price + backtest_config.min_stop_atr * atr,
            )
            ...
            stop_anchor = max(float(data["high"][breakout_idx]), float(data["high"][signal_idx]), range_high)
            stop_price = max(
                stop_anchor + backtest_config.failure_stop_buffer_atr * atr,
                entry_price + backtest_config.min_stop_atr * atr,
            )
            risk_per_unit = stop_price - entry_price
            target_mid = min(range_mid, entry_price - 0.10 * atr)
            target_price = max(
                target_mid,
                entry_price - risk_per_unit * backtest_config.failure_target_r_multiple,
            )
            max_holding = backtest_config.failure_max_holding_bars
        else:
            stop_anchor = min(float(data["low"][breakout_idx]), float(data["low"][signal_idx]), range_low)
            stop_price = min(
                stop_anchor - backtest_config.failure_stop_buffer_atr * atr,
                entry_price - backtest_config.min_stop_atr * atr,
            )
            risk_per_unit = entry_price - stop_price
            target_mid = max(range_mid, entry_price + 0.10 * atr)
            target_price = min(
                target_mid,
                entry_price + risk_per_unit * backtest_config.failure_target_r_multiple,
            )
    ...
    if trades:
        ...
        trades_df = pl.DataFrame(
            schema={
                ...
                "range_window_start_idx": pl.Int64,
                "range_window_end_idx": pl.Int64,
                "range_high": pl.Float64,
                "range_low": pl.Float64,
                "range_mid": pl.Float64,
                "range_height": pl.Float64,
                "range_height_atr": pl.Float64,
                "range_context_at_breakout": pl.Boolean,
                ...
                "breakout_close_pos": pl.Float64,
                "breakout_body_ratio": pl.Float64,
                "signal_close_pos": pl.Float64,
                "signal_body_ratio": pl.Float64,
...
def execute_preset(
    ...
    df = detect_breakout(df_source, config=detect_config)
    df = label_breakout_outcomes(df, config=detect_config)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="突破与失败突破策略回测。")
...
def main() -> None:
    ...
    combined_summary: dict[str, Any] = {
        "source_docs": [
            "阿布课程语音转文字/15A 突破.txt",
            "阿布课程语音转文字/15B 突破.txt",
        ],
